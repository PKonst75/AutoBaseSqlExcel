...
{
    public class DbPrintWarrantV02 : DbPrint
    {
        ...
        protected class HeaderData
        {
            ...
            public HeaderData(DtCard card, int thetype1)
            {
                ...
                // Исключаем все негарантийные работы
                ArrayList del_works = new ArrayList();
                foreach (DtCardWork element in works)
                {
                    if (element.GuaranteeFlag() != true) del_works.Add(element);
                }
                foreach (DtCardWork to_del in del_works)
                {
                    works.Remove(to_del);
                }
                ...
                // Исключаем все негарантийные детали
                ArrayList del_details = new ArrayList();
                foreach (DtCardDetail element in details)
                {
                    if (element.IsGuaranty != true) del_details.Add(element);
                }
                foreach (DtCardDetail to_del in del_details)
                {
                    details.Remove(to_del);
                }
        ...
        public DbPrintWarrantV02(long card_number, int card_year, int type)
        {
            thetype = type;
        ...
        private int PrintHead(Graphics graph, int offset, bool test, bool print, object o)
        {
            ...
            {
                PrintText(graph, "ЗАКАЗ-НАРЯД №" + header_data.txt_warrant_number + "Г", header_x, y, 150, 5, StringAlignment.Near, StringAlignment.Center, font_middle_bold, brush_standart, false);
            ...
            else
            {
                if (header_data.txt_warrant != "")
                    PrintText(graph, "ЗАКАЗ-НАРЯД №" + header_data.txt_warrant + "Г", header_x, y, 150, 5, StringAlignment.Near, StringAlignment.Center, font_middle_bold, brush_standart, false);
                else
            ...
            y += height;
            // Телефон
            PrintTextBox(graph, "Телефон | e-mail:", header_x, y, header_title_width, header_line_height, text_offset, StringAlignment.Near, StringAlignment.Center, font_small_bold, brush_standart, pen_thin, false);
            PrintTextBox(graph, header_data.txt_phone + " | " + header_data.txt_mail, header_x + header_title_width, y, header_data_width, header_line_height, text_offset, StringAlignment.Near, StringAlignment.Center, font_small_bold, brush_standart, pen_thin, false);
            y += header_line_height;
            // Доверенное лицо
