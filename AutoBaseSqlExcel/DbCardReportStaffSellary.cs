...
{
	class DbCardReportStaffSellary
	{
		...
		/* Новая методика май 2023 */
		private float _sellaryHours;
		private float _salaryServiceMechanic;
		private float _salaryServiceManager;
		private float _paidAmount;



		public DbCardReportStaffSellary(DbCard cardSrc)
		{
			...
			foreach (object o in cardWorks)
			{
				...
				/* Новая система расчета  май 2023 */
				_sellaryHours += cardWork.SalaryHours();
				_salaryServiceMechanic += cardWork.SalaryServiceMechanic();
				_salaryServiceManager += cardWork.SalaryServiceManager(_dtCard.ServiceManager);
				_paidAmount += (cardWork.PaidAmount() - _dtCard.Discount* cardWork.PaidAmount() / 100);
			}
			// Сумма деталей
		...
		public float SellaryHours
		{
			get { return _sellaryHours; }
		}
		public float SalaryServiceMechanic
		{
			get { return _salaryServiceMechanic; }
		}
		public float PaidAmount
		{
			get { return _paidAmount; }
		}
		public float SalaryServiceManager
		{
			get { return _salaryServiceManager; }
		}
		public string ServiceManager
		{
			get {
				if (_dtCard.ServiceManager == null)
					return "нет";
				else
					return _dtCard.ServiceManager.Title;
			}
		}
		public string Department
		{
			get { return _dtCard.Workshop.Title(); }
		}
	}
}
