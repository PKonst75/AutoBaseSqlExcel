...
namespace AutoBaseSql
{
	...
	public class UIF_Alarm_List : System.Windows.Forms.Form
	{
		...
		private void button_new_Click(object sender, System.EventArgs e)
		{
			// Новая сигнализация
			string txt = "";
			txt = UserInterface.Selector_String("Введите наименование сигнализации", "");
			if(txt == "") return;

			// Попытка записать новую сигнализацию
			DtListAlarm alarm = new DtListAlarm();
			alarm.SetData("НАИМЕНОВАНИЕ", (object)txt);
			alarm = (DtListAlarm)DbSqlListAlarm.Insert(alarm);
			if(alarm == null) return;
			long code = (long)alarm.GetData("КОД_СИГНАЛИЗАЦИЯ");
