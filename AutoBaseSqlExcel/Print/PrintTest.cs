...
{
    class PrintTest: PrintCommonHeadTest
	{
        ...
        public PrintTest(DtCard srcCard, CalculatorCard srcCalculatorCard) : base(srcCard)
        {
        ...
        public float BlockDateTime(float offset, object print_data, int coutCounter = 0)
        {
            ...
            y += _printTools.SelectedPrintTableGroup(line_data, PAGE_LEFT_X, y, BLOCKDATETIME_ROW_HEIGHT);
		...
		public float BlockClaim(float offset, object print_data = null, int coutCounter = 0)
		{
			...
			return offset + _printTools.SelectedPrintTextBox(txt, PAGE_LEFT_X, offset, PAGE_RIGHT_X - PAGE_LEFT_X, BLOCK_CLAIM_HEIGHT);
		}
		...
		public float BlockClaimWarning(float offset, object print_data = null, int coutCounter = 0)
		{
			...
			_printTools.SetStringFormat(StringAlignment.Near, StringAlignment.Near);
			return offset + _printTools.SelectedPrintTextBox(txt, PAGE_LEFT_X, offset, PAGE_RIGHT_X - PAGE_LEFT_X, BLOCK_CLAIMWARNING_HEIGHT);
		}
		...
		public float BlockTotalSumm(float offset, object print_data = null, int coutCounter = 0)
		{
			...
			_printTools.SelectFont(_fontArial8Bold);
			_printTools.SetStringFormat(StringAlignment.Near, StringAlignment.Near);
			return y + _printTools.SelectedPrintTextNoBox(txt, PAGE_LEFT_X, y, PAGE_RIGHT_X - PAGE_LEFT_X, BLOCK_TOTALSUMM_HEIGHT);
		}
		...
		public float BlockGuaranty(float offset, object print_data = null, int coutCounter = 0)
		{
			...
			_printTools.SelectFont(_fontArial6);
			_printTools.SetStringFormat(StringAlignment.Near, StringAlignment.Near);
			return y + _printTools.SelectedPrintTextBox(txt, PAGE_LEFT_X, y, PAGE_RIGHT_X - PAGE_LEFT_X, BLOCK_GUARANTY_HEIGHT);
		}
		...
		public float BlockQuolity(float offset, object print_data = null, int coutCounter = 0)
		{
			...
			_printTools.SelectFont(_fontArial6);
			_printTools.SetStringFormat(StringAlignment.Near, StringAlignment.Near);
			return y + _printTools.SelectedPrintTextNoBox(txt, PAGE_LEFT_X, y, PAGE_RIGHT_X - PAGE_LEFT_X, BLOCK_QUOLITY_HEIGHT);
		}
		...
		public float BlockRecomendation(float offset, object print_data = null, int coutCounter = 0)
		{
			...
			_printTools.SelectFont(_fontArial6);
			_printTools.SetStringFormat(StringAlignment.Near, StringAlignment.Near);
			y += _printTools.SelectedPrintTextNoBox(txt, PAGE_LEFT_X, y, PAGE_RIGHT_X - PAGE_LEFT_X, BLOCK_RECOMEND_HEIGHT);
			txt = "Рекомендации по поддержанию автомобиля в технически исправном состоянии.";
			y += _printTools.SelectedPrintTextNoBox(txt, PAGE_LEFT_X, y, PAGE_RIGHT_X - PAGE_LEFT_X, BLOCK_RECOMEND_HEIGHT);
			txt = _cardTxt.RecomendationsList;
			y += _printTools.SelectedPrintTextBox(txt, PAGE_LEFT_X, y, PAGE_RIGHT_X - PAGE_LEFT_X, BLOCK_RECOMEND_HEIGHT);
		...
		public float BlockFinal(float offset, object print_data = null, int coutCounter = 0)
		{
			...
			_printTools.SelectFont(_fontArial6);
			_printTools.SetStringFormat(StringAlignment.Near, StringAlignment.Near);
			y += _printTools.SelectedPrintTextNoBox(txt, PAGE_LEFT_X, y, PAGE_RIGHT_X - PAGE_LEFT_X, BLOCK_FINAL_HEIGHT);
			txt = "Заказчик ___________________ /" + _cardTxt.RepresentTitleShort + "/";
			_printTools.SelectedPrintTextNoBox(txt, PAGE_LEFT_X, y, BLOCK_FINAL_RINGHTPART_X - PAGE_LEFT_X, BLOCK_FINAL_HEIGHT);
			txt = "Мастер Консультант ___________________ /" + _cardTxt.ServiceManagerShortName +"/";
			y += _printTools.SelectedPrintTextNoBox(txt, BLOCK_FINAL_RINGHTPART_X, y, PAGE_RIGHT_X - BLOCK_FINAL_RINGHTPART_X, BLOCK_FINAL_HEIGHT);
			txt = "Дата печати документа " + DateTime.Now.ToShortDateString();
			y += _printTools.SelectedPrintTextBox(txt, PAGE_RIGHT_X - BLOCK_FINAL_DATEWIDTH, y, BLOCK_FINAL_DATEWIDTH, BLOCK_FINAL_HEIGHT);
			return y;
		}
        #endregion
        #region Работы
        private float PrintWorkBlock(float offset, object o, int count)
		{
			...
			if (work.GuaranteeFlag())
			{
				...
				line_data[2] = new object[] { txtCardWork.WorkName, WORK_WIDTH_COLUMN3, PrintDrawingBase.ALIGN_X.LEFT };
			...
			_printTools.SelectFont(_fontArial6);
			y += _printTools.SelectedPrintTableGroup(line_data, PAGE_LEFT_X, y, ROW_HEIGHT);
			return y;
		}
		...
		private float PrintWorkBlockHead(float offset, object o = null, int count = 0)
		{
			...
			_printTools.SetStringFormat(StringAlignment.Center, StringAlignment.Center);
			_printTools.SelectFont(_fontArial6Bold);

			y += _printTools.SelectedPrintTextBox("Выполненные работы", PAGE_LEFT_X, y, PAGE_RIGHT_X - PAGE_LEFT_X, ROW_HEIGHT);
		...
		private float PrintDetailBlockHead(float offset, object o = null, int count = 0)
		{
			...
			y += _printTools.SelectedPrintTextBox("Расходная накладная к заказ-наряду", PAGE_LEFT_X, y, PAGE_RIGHT_X - PAGE_LEFT_X, ROW_HEIGHT);
		...
		private float PrintDetailBlock(float offset, object o, int count)
		{
			...
			if (detail.GuaranteeFlag())
			{
				...
				line_data[2] = new object[] { txtCardDetail.WorkName, DETAIL_WIDTH_COLUMN3, PrintDrawingBase.ALIGN_X.LEFT };
		...
		private float PrintDetailBlockTotal(float offset, object o = null, int count = 0)
		{
			...
			line_data[0] = new object[] { "ИТОГО", DETAIL_WIDTH_COLUMN1 + DETAIL_WIDTH_COLUMN2 + DETAIL_WIDTH_COLUMN3, PrintDrawingBase.ALIGN_X.RIGHT };
		...
		public override DocumentPrintSchema CreatePrintingSchema()
		{
			...
			delegates.Add(new DocumentPrintSchemaElement(BlockDateTime));
			delegates.Add(new DocumentPrintSchemaElement(BlockClaim));
			delegates.Add(new DocumentPrintSchemaElement(BlockClaimAnalize));
			...
			if (_card.CardWorks.Count > 0)
			{
				...
				{
					delegates.Add(new DocumentPrintSchemaElement(PrintWorkBlock, element, PrintWorkBlockHead, first, count));
					count++;
					first = false;
				}
				delegates.Add(new DocumentPrintSchemaElement(PrintWorkBlockTotal));
			}
			delegates.Add(new DocumentPrintSchemaElement(BlockClaimWarning));
			...
			if (_card.CardDetails.Count > 0)
			{
				...
				{
					delegates.Add(new DocumentPrintSchemaElement(PrintDetailBlock, element, PrintDetailBlockHead, first, count));
					count++;
					first = false;
				}
				delegates.Add(new DocumentPrintSchemaElement(PrintDetailBlockTotal));
			}

			delegates.Add(new DocumentPrintSchemaElement(BlockTotalSumm));
			delegates.Add(new DocumentPrintSchemaElement(BlockGuaranty));
			delegates.Add(new DocumentPrintSchemaElement(BlockQuolity));
			delegates.Add(new DocumentPrintSchemaElement(BlockRecomendation));
			delegates.Add(new DocumentPrintSchemaElement(BlockFinal));
