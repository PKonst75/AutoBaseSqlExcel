...
{
	/// <summary>
	/// Summary description for DbPrintGuarantyGM.
	/// </summary>
	public class DbPrintGuarantyGM:DbPrint
	{
		...
		protected class HeaderData
		{
            ...
            public string txt_warrant_close_short = "";
			public string txt_warrant_open		= "";
			public string txt_card_open			= "";
            public string txt_card_open_short = "";
			public DIALER dialer		= DIALER.unknown;
			public string txt_variant	= "";
			...
			public HeaderData(DtCard card)
			{
                ...
                txt_card_open_short = ((DateTime)(card.GetData("ДАТА"))).ToShortDateString();
				...
				if((short)card.GetData("СТАТУС_КАРТОЧКА") == 2)
				{
                    ...
                    DateTime dt = (DateTime)card.GetData("ДАТА_НАРЯД_ЗАКРЫТ_КАРТОЧКА");
                    txt_warrant_close_short = dt.ToShortDateString();
				}
				...
				if(auto != null)
				{
					...
					txt_variant = (string)auto.GetData("АВТОМОБИЛЬ_ИСПОЛНЕНИЕ");
					txt_color	= (string)auto.GetData("АВТОМОБИЛЬ_ЦВЕТ");
					...
					if(model != null)
						...
						// Добавляем вариант и цвет!
						if ((txt_variant != "")||(txt_color != ""))
							txt_model += " (" + txt_variant + " " + txt_color + ")";
					// Дата продажи
		...
		public DbPrintGuarantyGM(long card_number, int card_year)
		{
		...
		private int PrintHead(Graphics graph, int offset,  bool test, bool print, object o)
		{
			...
			int	header_title_width	= 35;		// Ширина блока заголовка для наименований 40
			int	header_data_width	= 80;		// Ширина блока заголовка для данных 75
			int x1					= 50;
            int x2 = 30; x2 += 20; // Новое
			if(test == true || print == false)
			{
				...
				y += header_line_height;
				y += header_line_height;
                y += header_line_height; // Дополнительное новое поле!
				...
				return y + 3 + 5;	// Отсуп от заголовка + 5 новое

			}

			y	= offset;
			// НАЗВАНИЕ ДОКУМЕНТА
			PrintText(graph, "ЗАКАЗ-НАРЯД №" + header_data.txt_warrant_number + "от" + header_data.txt_card_open_short, header_x, y, 150, 5, StringAlignment.Near, StringAlignment.Center, font_middle_bold, brush_standart, false);
            y += 4; // Новоое!!!
			PrintText(graph, "Дата обращения клиента с требованием на ремонт:", header_x + x1 - 50, y, 90, 5, StringAlignment.Far, StringAlignment.Center, font_small, brush_standart, false);
			...
			y += 3;
			PrintText(graph, "Открыт/Дата передачи автомобиля потребителем в ремонт", header_x + x1 - 50, y, 90, 5, StringAlignment.Far, StringAlignment.Center, font_small, brush_standart, false);
			...
			y += 3;
			PrintText(graph, "Закрыт/Дата устранения дефектов:", header_x + x1 - 50, y, 90, 5, StringAlignment.Far, StringAlignment.Center, font_small, brush_standart, false);
			...
			y += 3;
            //string outdate_txt = DateTime.Now.ToShortDateString();
            PrintText(graph, "Дата выдачи потребителю автомобиля из ремонта:", header_x + x1 - 50, y, 90, 5, StringAlignment.Far, StringAlignment.Center, font_small, brush_standart, false);
            PrintText(graph, header_data.txt_warrant_close_short/*outdate_txt*/, header_x + x1 + x2, y, 150, 5, StringAlignment.Near, StringAlignment.Center, font_small, brush_standart, false);
            //   PrintText(graph, "Дата выдачи потребителю из ремонта:", header_x + x1 - 50, y, 70, 5, StringAlignment.Far, StringAlignment.Center, font_small, brush_standart, false);
            //   PrintText(graph, header_data.txt_warrant_close, header_x + x1 + x2, y, 150, 5, StringAlignment.Near, StringAlignment.Center, font_small, brush_standart, false);
			...
			size	= MeasureTextBox(graph, header_data.txt_model, header_data_width, text_offset, StringAlignment.Near, StringAlignment.Near, font_small_bold);
			height	= (int)Math.Ceiling(size.Height);
			if(height < header_line_height) height = header_line_height;
			...
			PrintTextNoBox(graph, header_data.txt_model, header_x + header_title_width, y, header_data_width, height, text_offset, StringAlignment.Near, StringAlignment.Center, font_small_bold, brush_standart, pen_thin, false);
			y += height;//y += header_line_height;
			...
			RectangleF rect1	= new RectangleF(header_x, offset + 10 + 4 + 3, header_data_width + header_title_width, y - (offset + 10 + 4 + 3));
			PrintRoundBox(graph, rect1, pen_thin, 4);
			PrintLineVrt(graph, header_x + header_title_width, offset + 10 + 4 + 3, y - (offset + 10 + 4 + 3), pen_thin, false);
            ...
            // СТРОЧКА ПРО ГАРАНТИЙНЫЙ ЗАКАЗ НАРЯД
            PrintText(graph, "ГАРАНТИЙНЫЙ РЕМОНТ. КЛИЕНТОМ НЕ ОПЛАЧИВАЕТСЯ.", header_x, header_height_real, 200, 5, StringAlignment.Center, StringAlignment.Center, font_middle_bold, brush_standart, false);

            // ПЛЮС 4 НОВОЕ!
			return header_height_real + 3 + 5;
		}
		#endregion
		...
		private int PrintFooter(Graphics graph, int offset,  bool test, bool print, object o)
		{
            ...
            txt = "С объемом выполненых работ и рекомендациями ознакомлен. Работа выполнена в полном объеме. Претензий к выполненной работе не имею. Сданный в ремонт автомобиль получил.";
			PrintText(graph, txt, offset_x_left, y, w1, rowheight*2, StringAlignment.Near, StringAlignment.Center, font_middle, brush_standart, false);
			y += rowheight * 2;
			PrintText(graph, "Владелец/Представитель", offset_x_left, y, w2, rowheight, StringAlignment.Near, StringAlignment.Center, font_middle, brush_standart, false);
			y += rowheight;
			PrintLineHor(graph, offset_x_left + w2, y, w3 + w4 + w5, pen_thin, false);
